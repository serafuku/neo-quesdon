...
export default async function detectInstance(host: string) {
  ...
  const nodeInfoMeta: nodeinfoMeta = await fetch(`${parsedUrl.origin}/.well-known/nodeinfo`).then((r) => {
    if (!r.ok) {
  ...
  const nodeInfoLink = nodeInfoMeta.links.find((el) => el.rel === 'http://nodeinfo.diaspora.software/ns/schema/2.0');
