...
export class FollowingService {
  ...
  public async getFollowing(req: NextRequest, @JwtPayload tokenBody?: jwtPayload) {
    ...
    for (const f of follows) {
      const exist = await prisma.user.findUnique({ where: { handle: f.followeeHandle } });
      if (exist) {
        filteredDto.followingList.push({ followerHandle: f.followerHandle, follweeHandle: f.followeeHandle });
      }
    }
